...
class DataExtractor():
    ...
    def gen_class_id_list(class_id_list):
        with get_session() as session:
            be_pool = session.query(BasicElement).all()
            if 0 != len(be_pool):
                for i in range(0, len(be_pool)):
                    class_id = MatchRule.encode_class_id(be_pool[i].lv1_id, be_pool[i].lv2_id)
    ...
    def find_template_pool_in_db(class_id, template_dic_pool):
        ...
        if class_id not in template_dic_pool.keys():
            ...
            if template_dic_pool[class_id] is None:
                with get_session() as session:
                    be_pool = session.query(BasicElement).filter_by(lv1_id=lv1_id, lv2_id=lv2_id).all()
                    if 0 == len(be_pool):
                        ...
                        attr_rule_dic = {}
                        for j in range(0, len(be_pool)):
                            # Store non attr rule elements:
                            if u'attr_rule' not in be_pool[j].attr_val:
                                if be_pool[j].attr_name not in template_dic_pool[class_id].keys():
                                    template_dic_pool[class_id][be_pool[j].attr_name] = []
                                template_dic_pool[class_id][be_pool[j].attr_name].append(be_pool[j].attr_val)
                            # Store attr rule elements.
                            else:
                                #把所有attr_rule拼接在一起？
                                if be_pool[j].attr_name not in attr_rule_dic.keys():
                                    attr_rule_dic[be_pool[j].attr_name] = u'-attr_rule;'
                                attr_rule_dic[be_pool[j].attr_name] += be_pool[j].attr_val.replace(u'-attr_rule;', u'')
                        ...
                        logger.info('Find basic template. lv1_id = %d, lv2_id = %d,' % (lv1_id, lv2_id))
            ...
            for attr_name in template_dic_pool[class_id].keys():
                ...
                if u'str_format' in attr_name_rule_map[class_id][attr_name]:
                    ...
                    for j in range(0, len(rule_list)):
                        ...
                        elif u'-' in rule_list[j] and start_to_get_ex:
                            break
    ...
    def extract_product_info(self):
        ...
        for rd_row_idx in range(0, row_num):
            ...
            class_id = MatchRule.encode_class_id(int(rd_line[id_col_id]), 0, u'xls')
    ...
    def extract_product_info_by_ind(lv1_id, lv2_id, product_info, result):
        ...
        if class_id not in template_dic_pool.keys():
            logger.error('No template, can not extract product id %d %d.' % (lv1_id, lv2_id))
            return
        ...
        # Create attr_name <-> rule map.
        attr_name_rule_map = {}
        DataExtractor.gen_attr_name_rule_map(class_id, template_dic_pool, attr_name_rule_map)
